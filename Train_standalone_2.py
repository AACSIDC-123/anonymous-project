...
def train_server_overlap(client2_fx, Y_1, t, batch_id, correct_overlap, size):
    server_model.train()
    correct = correct_overlap
    _, server_fx_avg = server_model(client2_fx, client2_fx)
    ...
    loss.backward()
    dfx1 = client2_fx.grad.clone().detach().to(device)
...
def train_server_complete1(client2_fx, Y_1, t, batch_id, correct_complete1, size):
    server_model.train()
    correct = correct_complete1
    global train_complete1_acc 
    global train_complete1_loss

    _, server_fx1 = server_model(client2_fx, client2_fx)
...
def train_client(dataloader, client_model_2, t):
    ...
    for batch_id, batch in enumerate(dataloader):
        ...
        if args.overlap == 'True':
            ...
            g_fx2, correct_overlap = train_server_overlap(client2_fx,  Y_overlap, t, batch_id, correct_overlap, size)
        ...
        if args.complete_1 == 'True':
            ...
            fx2 = client_model_2(X_2_non_overlap)

            client2_fx = (fx2).clone().detach().requires_grad_(True)
            g_fx2, correct_complete1 = train_server_complete1(client2_fx, Y_non_overlap2, t, batch_id, correct_complete1, size)
        ...
        if (t+1) %10 ==0 or t == args.epochs-1:
            if batch_id == args.num_overlap-1:  
                save_path1 = f'Result/Results_standalone2/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/client1_epoch{t+1}.pth'
                save_path2 = f'Result/Results_standalone2/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/client2_epoch{t+1}.pth'
...
def test_server(client2_fx, partyB_zeros, Y_1, 
    ...
    optimizer_server.zero_grad()
    _, fx_server_normal = server_model(client2_fx, client2_fx)
    ...
    for i, partyB_zero in enumerate(partyB_zeros):
        optimizer_server.zero_grad()
        _, fx_server_zero = server_model(partyB_zero, partyB_zero)
...
def test_client(dataloader, client_model_2, t):
    client_model_2.eval()
...
if __name__ == '__main__':
    ...
    save_path = f'Result/Results_standalone2/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    filename = open(f'Result/Results_standalone2/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/A_training_process.txt', 'w+')
    ...
    if args.dataset =='MIMIC' or args.dataset == 'avazu' or args.dataset == 'bank':
        ...
        client_model_1, client_model_2, server_model = ImageModel2.GetModel()
    ...
    for t in range(args.epochs):
        ...
        train_client(train_data, client_model_2, t)
        test_client(test_data, client_model_2, t)
    ...
    #save
    np.save(f'Result/Results_standalone2/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/train_overlap_ACC_sum.npy',   train_overlap_acc) 
    np.save(f'Result/Results_standalone2/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/train_overlap_lOSS_sum.npy',  train_overlap_loss)
    np.save(f'Result/Results_standalone2/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/train_complete1_ACC_sum.npy',   train_complete1_acc) 
    np.save(f'Result/Results_standalone2/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/train_complete1_lOSS_sum.npy',  train_complete1_loss)
    ...
    plt.legend()
    plt.savefig(f'Result/Results_standalone2/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/A_train_ACC.png')
    plt.close()
