...
def train_server(client1_fx, Y_1, t, batch_id, correct, server_model, num_All, size):
    server_model.train()
    correct = correct

    _, server_result = server_model(client1_fx)
...
def train_client(dataloader, client_model_1, server_model, t):
    client_model_1.train()

    correct = 0

    # len(dataloader) indicates the number of batch
    ...
    for batch_id, batch in enumerate(dataloader):
        ...
        X_1_All = torch.cat([X_1_overlap, X_1_complete2, X_1_complete3, X_1_complete4], dim=0)
        ...
        Y_All = torch.cat([Y_overlap, Y_complete2, Y_complete3, Y_complete4], dim=0)
        ...
        num_All = args.num_overlap + num_complete2 + num_complete3 + num_complete4
        
        # client1--train and update
        fx1 = client_model_1(X_1_All)
        # Sending activations to server and receiving gradients from server
        client1_fx = (fx1).clone().detach().requires_grad_(True)
        g_fx1, correct = train_server(client1_fx, Y_All, t, batch_id, correct, server_model, num_All, size)
...
def test_server(dataloader, client_1_embeddings, Y_1, batch_id, correct, loss, server_model, size, t):
    ...
    for i, client_1_embedding in enumerate(client_1_embeddings):
        ...
        _, fx_server = server_model(client_1_embedding)
...
def test_client(dataloader, client_model_1, server_model, t):
    ...
    for batch_id, batch in enumerate(dataloader):
        ...
        # Sending activations to server and receiving gradients from server
        correct, loss = test_server(dataloader, client_1_embeddings, Y_1, batch_id, correct, loss, server_model, size, t)
...
if __name__ == '__main__':
    ...
    save_path = f'Result/Results_standalone1_4clients/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    filename = open(f'Result/Results_standalone1_4clients/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/A_training_process.txt', 'w+')
    ...
    for t in range(args.epochs):
        ...
        train_client(train_data, client_model_1, server_model, t)
        test_client(test_data, client_model_1, server_model, t)
    ...
    plt.legend()
    plt.savefig(f'Result/Results_standalone1_4clients/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/A_train_acc.png')
    plt.close()
