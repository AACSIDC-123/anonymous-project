...
def train_server(client2_fx, Y_1, t, batch_id, correct, server_model, num_All, size):
    server_model.train()
    correct = correct

    _, server_result = server_model(client2_fx)
...
def train_client(dataloader, client_model_2, server_model, t):
    ...
    for batch_id, batch in enumerate(dataloader):
        ...
        X_2_All = torch.cat([X_2_overlap, X_2_complete1, X_2_complete3, X_2_complete4], dim=0)
        ...
        Y_All = torch.cat([Y_overlap, Y_complete1, Y_complete3, Y_complete4], dim=0)
        ...
        num_All = args.num_overlap + num_complete1 + num_complete3 + num_complete4
        
        # client1--train and update
        fx2 = client_model_2(X_2_All)
        # Sending activations to server and receiving gradients from server
        client2_fx = (fx2).clone().detach().requires_grad_(True)
        g_fx2, correct = train_server(client2_fx, Y_All, t, batch_id, correct, server_model, num_All, size)
        ...
        if (t+1) %10 ==0 or t == args.epochs-1:
            if batch_id == args.num_overlap-1:  
                # save_path1 = f'Result/Results_standalone2_4clients/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/client1_epoch{t+1}.pth'
                save_path2 = f'Result/Results_standalone2_4clients/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/client2_epoch{t+1}.pth'
...
def test_server(dataloader, client_2_embeddings, Y_1, batch_id, correct, loss, server_model, size, t):
    ...
    for i, client_2_embedding in enumerate(client_2_embeddings):
        ...
        optimizer_server.zero_grad()
        _, fx_server = server_model(client_2_embedding)
...
def test_client(dataloader, client_model_2, server_model, t):
    ...
    for batch_id, batch in enumerate(dataloader):
        ...
        # Sending activations to server and receiving gradients from server
        correct, loss = test_server(dataloader, client_2_embeddings, Y_1, batch_id, correct, loss, server_model, size, t)
...
if __name__ == '__main__':
    ...
    save_path = f'Result/Results_standalone2_4clients/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    filename = open(f'Result/Results_standalone2_4clients/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/A_training_process.txt', 'w+')
    ...
    for t in range(args.epochs):
        ...
        train_client(train_data, client_model_2, server_model, t)
        test_client(test_data, client_model_2, server_model, t)
    ...
    #save
    np.save(f'Result/Results_standalone2_4clients/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/train_acc.npy', train_acc) 
    ...
    plt.legend()
    plt.savefig(f'Result/Results_standalone2_4clients/Results_batchsize{args.batch_size}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/A_train_acc.png')
    plt.close()
