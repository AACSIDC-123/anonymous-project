...
def train_server_overlap(client1_student_fx, client1_teacher_fx, client2_teacher_fx, Y_1, t, batch_id, correct_overlap, size, server_teacher_model, server_student_model):
    ...
    T = 2.0  
    _, server_student = server_student_model(client1_student_fx, client1_student_fx)
    ...
    loss.backward()
    dfx1 = client1_student_fx.grad.clone().detach().to(device)
...
def train_server_complete2(client1_fx, Y_1, t, batch_id, correct_complete2, server_student_model, size):
    server_student_model.train()
    correct = correct_complete2

    _, server_fx1 = server_student_model(client1_fx, client1_fx)
...
def train_client(dataloader, client_student_model_1, client_student_model_2, server_student_model, \
    ...
    for batch_id, batch in enumerate(dataloader):
        ...
        if args.overlap == 'True':
            ...
            client1_student_fx = (fx1_student).clone().detach().requires_grad_(True)
            ...
            g_fx1, correct_overlap = train_server_overlap(client1_student_fx, client1_teacher_fx, client2_teacher_fx, Y_overlap, t, batch_id, correct_overlap, size, server_teacher_model, server_student_model)
            
            optimizer_student_client1.zero_grad()
            (fx1_student).backward(g_fx1)
        ...
        if args.complete_2 == 'True':
            ...
            g_fx1, correct_complete2 = train_server_complete2(client1_fx, Y_non_overlap1, t, batch_id, correct_complete2, server_student_model, size)
        ...
        # record 
        if batch_id == args.num_batch -1:
            save_path1 = f'Result/Results_IAVFL_1/Results_batchsize{args.batch_size}/learningRate{args.lr}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/client1.pth'
            save_path2 = f'Result/Results_IAVFL_1/Results_batchsize{args.batch_size}/learningRate{args.lr}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/client2.pth'
            save_path3 = f'Result/Results_IAVFL_1/Results_batchsize{args.batch_size}/learningRate{args.lr}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/server.pth'
...
def test_server(client1_fx, partyA_zeros, Y_1, 
    ...
    optimizer_student_server.zero_grad()
    _, fx_server_normal = server_student_model(client1_fx, client1_fx)
    ...
    for i, partyA_zero in enumerate(partyA_zeros):
        optimizer_student_server.zero_grad()
        _, fx_server_zero = server_student_model(partyA_zero, partyA_zero)
    ...
    if batch_id == len(test_data) - 1:
        ...
        if correct_normal_ratio > best_correct_ratio:
            ...
            save_path1_best = f'Result/Results_IAVFL_1/Results_batchsize{args.batch_size}/learningRate{args.lr}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/client1_best.pth'
            save_path2_best = f'Result/Results_IAVFL_1/Results_batchsize{args.batch_size}/learningRate{args.lr}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/client2_best.pth'
            save_path3_best = f'Result/Results_IAVFL_1/Results_batchsize{args.batch_size}/learningRate{args.lr}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/server_best.pth'
...
def test_client(dataloader, client_student_model_1, client_student_model_2, server_student_model, t):
    ...
    for batch_id, batch in enumerate(dataloader):
        ...
        fx1 = client_student_model_1(X_1)
        client1_fx = (fx1).clone().detach().requires_grad_(True)
        ...
        for i, missing_ratio in enumerate(missing_ratios):
            ...
            fx1_blank = client_student_model_1(X_1_blank)
...
if __name__ == '__main__':
    ...
    save_path = f'Result/Results_IAVFL_1/Results_batchsize{args.batch_size}/learningRate{args.lr}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/'
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    filename = open(f'Result/Results_IAVFL_1/Results_batchsize{args.batch_size}/learningRate{args.lr}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/A_training_process.txt', 'w+')
    ...
    #save
    np.save(f'Result/Results_IAVFL_1/Results_batchsize{args.batch_size}/learningRate{args.lr}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/train_overlap_ACC.npy',   train_overlap_acc) 
    np.save(f'Result/Results_IAVFL_1/Results_batchsize{args.batch_size}/learningRate{args.lr}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/train_overlap_lOSS.npy',  train_overlap_loss)
    np.save(f'Result/Results_IAVFL_1/Results_batchsize{args.batch_size}/learningRate{args.lr}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/train_complete2_ACC.npy',   train_complete2_acc) 
    np.save(f'Result/Results_IAVFL_1/Results_batchsize{args.batch_size}/learningRate{args.lr}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/train_complete2_lOSS.npy',  train_complete2_loss)
    ...
    x2 = np.arange(len(train_complete2_acc))
    x3 = np.arange(len(test_normal_acc))

    plt.plot(x1, train_overlap_acc,   label='train_overlap_acc')
    plt.plot(x2, train_complete2_acc, label='train_complete2_acc')
    ...
    plt.legend()
    plt.savefig(f'Result/Results_IAVFL_1/Results_batchsize{args.batch_size}/learningRate{args.lr}/Seed{args.seed}/nonoverlap1_ratio_{args.nonoverlap1_ratio}/{args.dataset}/iclasses{args.classes}/{args.model}-level{args.level}/client{args.number_client}-c{args.num_cutlayer}/samples{args.num_batch*args.batch_size}/overlap{args.num_overlap*args.num_batch}/A_train_ACC.png')
    plt.close()
